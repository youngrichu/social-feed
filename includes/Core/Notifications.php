...
class Notifications {
    ...
    private function check_church_app_available() {
        if (!defined('SOCIAL_FEED_CHURCH_APP_AVAILABLE') || !SOCIAL_FEED_CHURCH_APP_AVAILABLE) {
            error_log('Social Feed: Church App Notifications plugin is not available');
            return false;
        }

        if (!class_exists('Church_App_Notifications')) {
            error_log('Social Feed: Church App Notifications class not found');
            return false;
        }

        // Check if Expo Push class exists
        if (!defined('SOCIAL_FEED_EXPO_PUSH_PATH') || !file_exists(SOCIAL_FEED_EXPO_PUSH_PATH)) {
            error_log('Social Feed: Expo Push class not found at expected path');
            return false;
        }
        ...
        if (!$tokens_exist || !$notifications_exist) {
            error_log('Social Feed: Required database tables are missing');
            return false;
        }
    ...
    /**
     * Check for new content and send notifications with improved reliability
    ...
    public function check_and_send_notifications() {
        ...
        try {
            ...
            foreach ($new_videos as $video) {
                ...
                // Add retry logic for individual notifications
                $notification_sent_success = false;
                $max_attempts = 3;
                ...
                for ($attempt = 1; $attempt <= $max_attempts; $attempt++) {
                    if ($this->send_notification($notification)) {
                        ...
                        $notifications_sent++;
                        $notification_sent_success = true;
                        break;
                    } else {
                        error_log("Social Feed: Notification attempt {$attempt} failed for video: " . $content->title);
                ...
                if (!$notification_sent_success) {
                    $notifications_failed++;
                    error_log("Social Feed: All notification attempts failed for video: " . $content->title);
                }
            }

            // Update last check time only if we processed successfully
            update_option('social_feed_last_notification_check', current_time('mysql'));

            // Clear retry count on successful check
            delete_transient('social_feed_notification_retry_count');

            // Log successful check
            error_log("Social Feed: Notification check completed. Found " . count($new_videos) . " new videos, sent {$notifications_sent} notifications, {$notifications_failed} failed.");

            // Store notification stats for monitoring
            $stats = get_option('social_feed_notification_stats', []);
            $stats[date('Y-m-d H:i')] = [
                'videos_found' => count($new_videos),
            ...
            if (count($stats) > 100) {
                $stats = array_slice($stats, -100, null, true);
            }
            
            update_option('social_feed_notification_stats', $stats);

            // Trigger refresh action
            do_action('social_feed_content_refreshed');
            
        } catch (\Exception $e) {
            error_log('Social Feed: Error in check_and_send_notifications: ' . $e->getMessage());
            error_log('Social Feed: Stack trace: ' . $e->getTraceAsString());
            
            // Implement retry logic with exponential backoff
            $retry_count = get_transient('social_feed_notification_retry_count') ?: 0;
            $max_retries = 3;
            
            if ($retry_count < $max_retries) {
                $retry_delay = min(300 * pow(2, $retry_count), 1800); // Max 30 minutes delay
                set_transient('social_feed_notification_retry_count', $retry_count + 1, 3600);
                wp_schedule_single_event(time() + $retry_delay, 'social_feed_check_notifications');
                error_log("Social Feed: Scheduled notification retry #{$retry_count} in {$retry_delay} seconds");
            } else {
                error_log('Social Feed: Max notification retries exceeded, will try again on next scheduled run');
                delete_transient('social_feed_notification_retry_count');
            }
        }
    }
    ...
    public function test_notifications($type = 'both') {
        ...
        try {
            ...
            if (!self::$initialized || !self::$church_app_available) {
                throw new \Exception('Notifications system not initialized or Church App not available');
            }

            // Check for active device tokens
            global $wpdb;
            $tokens_table = $wpdb->prefix . 'app_push_tokens';
